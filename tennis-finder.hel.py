...
#       [
#        { date: thedate,
#          availabilities: [{'time':xx, 'duration': yy, 'court': zz}, ..]
#                          # = parse_results()
#        },
#        ..
#       ],
# }
all_results: dict = {cname: [] for cname in CENTERS}


def fetch_raw_data(
    baseurl: str, the_date: str, book_path: str | None = '/booking/booking-calendar'
) -> str | None:
    """Fetches web-page
        See https://www.scrapehero.com/
        how-to-fake-and-rotate-user-agents-using-python-3/

    Args:
        baseurl (str): base url
        the_date (str): date parameter in 'YYYY-MM-DD' format
        book_path (str|None): booking path of the url.
            Defaults to '/booking/booking-calendar'.
...
def parse_results(data: str | None, center: str) -> list:
    ...
    Returns:
        list: list of availabilities for the center and date;
        each availability is a dic()
    """
    ...
    for link in links:
        urlstring: str = urlparse(link, allow_fragments=False).query
        urldic: dict = parse_qs(urlstring)
        if urldic is None:
            continue
        court_no: int = int(urldic.get('resid')[0])
        duration: str = urldic.get('kesto')[0]
        start_time: datetime = datetime.strptime(
            urldic.get('alkuaika')[0], '%Y-%m-%d %H:%M:%S'
        )
        start_hour: int = start_time.hour

        is_suitable_time: bool = start_hour in range(START_HOUR, END_HOUR + 1)
...
def tennis_finder_hel() -> None:
    ...
    print(
        ...
        f' with starting time between {START_HOUR} and {END_HOUR}...\n'
    )
    today: date = date.today()
    # check all days from today up to +PLUS_DAYS_MAX
    for days_plus in range(PLUS_DAYS_MAX + 1):
        date_: date = today + timedelta(days=days_plus)
        # date_nice is used in URL and in presenting the data
        date_for_url: str = date_.strftime('%Y-%m-%d')  # for forming URL
        date_nice: str = date_.strftime('%a %d %b')  # for printout
        # skip weekends
        if date_.isoweekday() in WEEKENDS:
            # logging.debug(f'Date {date_} (day = {date_.isoweekday()}) -->
            # Weekend detected, skipping')
            continue
        # check all tennis centers
        for center, cdata in CENTERS.items():
            url: str = cdata.get('url', '')
            # logging.debug(f'fetching: {date_}: {url}')
            data: str | None = fetch_raw_data(url, date_for_url)
            parsed_data = parse_results(data, center)
            print(
                f'Availability for {center.title()} center '
                f"({CENTERS.get(center).get('url')}) on {date_} between "
                f'{START_HOUR} and {END_HOUR}: {len(parsed_data)} slots.'
            )
            if parsed_data:
...
if __name__ == '__main__':
    print(
        'This program finds tennis centers availability for the upcoming days '
        'in Helsinki.\n'
    )
    tennis_finder_hel()
